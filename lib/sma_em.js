...
function restartClientIfLost(self) {

    if (self.options.debug) {
        console.log(`Checking client status...`);
    }
    let timestamp = Date.now();
    if (self.client && (self.lastReading + (self.options.refreshInterval * 2000) < timestamp)) {
        if (self.options.debug) {
            console.log(`No messages recieved for some time. Restarting client listener...`);
        }
        self.shouldBeConnected = false;
        self.client.close();
        initListenersAndConnect(self);
...
function initListenersAndConnect(self) {
    ...
    self.client.on('listening', () => {
        self.intervalTimer = setInterval(restartClientIfLost, self.options.refreshInterval * 2000, self);
    });

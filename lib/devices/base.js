...
class Base extends HomeyEventEmitter {
  ...
  async #initListenersAndConnect() {
    ...
    this.client = new modbus.client.TCP(this.#socket, 3, this.#resolveTimeoutMs());
  ...
  async #readProperties() {
    ...
    try {
      ...
      if (decodeData.isDeviceNotReady(decodedType)) {
        this.emit('error', new Error('Device type not available yet (device is booting or asleep); will retry on next poll'));
        return { ok: false, poisoned };
      }
  ...
  //   `poisoned` (so the sweep-level logic can reset the socket after repeated
  //   failures) and zero-fills the run - it does NOT reset the socket here or
  //   fall back to per-register reads, which would only pile more doomed
  //   requests onto an already-desynced socket.
    ...
    for (const run of runs) {
      ...
      try {
        ...
        if (util.isConnectionPoisoningError(runErr)) {
          // Connection desynced - flag it, zero-fill the run and move on. Let the
          // sweep-level poison handling decide whether to reset the socket.
          poisoned = true;
          this.emit('error', new Error(`Failed to read range ${run.start}+${run.count} for device type '${this.#deviceType}'`));
          for (const reg of run.registers) {
  ...
  async #readRegistersIndividually(registers, label) {
    ...
    for (const reg of registers) {
      ...
      if (reading.poison) {
        // Connection-level failure - don't track as a register-specific fault.
        poisoned = true;

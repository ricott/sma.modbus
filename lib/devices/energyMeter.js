...
function readObisValues(message, response) {
    ...
    for (let pointer = 28; pointer < message.length; pointer++) {
        const val = message.readUIntBE(pointer, obis_dword_length);
        const potentialObis = decodeObis(val);
...
function readSWVersion(message, pointer) {
    const value = message.readUInt32BE(pointer + obis_dword_length);
    const b0 = value & 0xff;

...
/**
 * Report an inverter model the app doesn't recognise (the `default` branch of
 * the device-type decoder). Rate-limited to once per day per type id (per Homey)
 * so it builds a low-volume inventory of unsupported models: each distinct type
 * id becomes its own Sentry issue, and its "Users" count shows how many Homeys
 * have that unsupported model. No-op when disabled.
 * @param {number|string} typeId
 */
function reportUnknownDeviceType(typeId) {
    report(
        `unknown-inverter-model:${typeId}`,
        `Unknown inverter model (type ${typeId})`,
        {
            level: 'info',
            tags: { unknownModelTypeId: String(typeId) },
            minIntervalMs: 24 * 60 * 60 * 1000 // once/day per type id
        }
    );
}
...
module.exports = { init, isEnabled, report, reportUnknownDeviceType, captureException, flush };

...
// SMA writes 0xFFFFFD ("Information not available") into an enum/taglist
// register when the value is not ready yet - e.g. an inverter still booting or
// asleep in night mode. The device-type register (30053) is such a register, so
// a reachable-but-not-ready device answers the model read with this sentinel
// instead of a model id.
const SMA_NOT_AVAILABLE = 0xFFFFFD; // 16777213

// Marker returned by decodeDeviceType() when the device answered the model read
// but reported "information not available" (see SMA_NOT_AVAILABLE). Callers must
// treat this as "not identified yet, retry later" - NOT as an unsupported model.
const DEVICE_NOT_READY = 'DEVICE_NOT_READY';
exports.DEVICE_NOT_READY = DEVICE_NOT_READY;

exports.isDeviceNotReady = function (deviceType) {
    return deviceType === DEVICE_NOT_READY;
};
...
function decodeDevice(typeId) {
    // A reachable device that isn't ready yet (booting / night mode) reports the
    // "information not available" sentinel instead of a model id. That is not an
    // unsupported model, so signal 'not ready' (the caller retries) and do NOT
    // file an unknown-model report that would pollute the unsupported-model
    // inventory with sleeping-but-supported inverters.
    if (typeId === SMA_NOT_AVAILABLE) {

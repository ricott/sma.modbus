...
const { STP, STP_TL10, STP_JP, STP_TLEE, STP_TLEE_JP, STP_40, STP_SE } = require('./registry/stp.js');
const { SB, SB_JP22, SB_TLST21, SB_SE10, SB_4041 } = require('./registry/sb.js');
const { SHP_20 } = require('./registry/shp.js');
const { SBS, SBS_VL, SI, SI11 } = require('./registry/sbs.js');
const { BASIC } = require('./registry/basic.js');
const { TRIPOWERX, CORE1 } = require('./registry/tripower.js');
...
exports.getReadingRegistries = function (modbusSettings) {
    ...
    Object.values(modbusSettings).forEach(function (registry) {
        if (registry != null && registry.setting === setting.READING) {
            arr.push({ registryId: registry.registryId, registerCount: registry.registerCount });
        }
    });
    return arr;
}
